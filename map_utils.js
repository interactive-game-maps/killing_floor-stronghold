{ // Helper functions
    function add_checkbox_for_marker(feature, marker, list, list_name, cluster) {
        ...
        checkbox.id = list_name + ':' + feature.properties.number;
        ...
        checkbox.addEventListener('change', () => {
            if (checkbox.checked) {
                ...
                localStorage.setItem(list_name + ":" + feature.properties.number, true);
            } else {
                marker.addTo(cluster);
                // remove from localStorage
                localStorage.removeItem(list_name + ":" + feature.properties.number);
            }
        });

        // hide if checked previously
        if (localStorage.getItem(list_name + ":" + feature.properties.number)) {
